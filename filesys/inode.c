...
struct inode_disk {
    ...
    uint32_t unused[124]; /* Not used. */ // padding
};

/* Returns the number of sectors to allocate for an inode SIZE
 * bytes long. */
static inline size_t
bytes_to_sectors(off_t size) { // byte / sector 
...
byte_to_sector(const struct inode *inode, off_t pos) {
    ...
    if (pos >= inode->data.length) // 유효 범위인지 확인 
        return -1;

    cluster_t clst = sector_to_cluster(inode->data.start); // clst check
